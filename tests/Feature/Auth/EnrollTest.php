...
class EnrollTest extends TestCase
{
    ...
    public function test_user_can_view_other_journals_to_enroll_in_profile_roles_tab(): void
    {
        ...
        // Assert Journal B (not enrolled) is visible with "Not Enrolled"
        $response->assertSee($journalB->name);
        $response->assertSee('Not Enrolled');
 
        // Assert Journal C (already enrolled) is visible with "Enrolled"
        $response->assertSee($journalC->name);
        $response->assertSee('Enrolled');
        
        // Assert "Manage Roles" is present
    ...
    public function test_user_can_sync_roles_via_ajax(): void
    {
        // 1. Create two journals
        $journalA = Journal::factory()->create(['enabled' => true, 'slug' => 'journal-a']);
        $journalB = Journal::factory()->create(['enabled' => true, 'slug' => 'journal-b']);
        ...
        // 2. Create user and enroll in Journal A
        $user = User::factory()->create();
        $authorRoleA = Role::where('journal_id', $journalA->id)->where('name', 'Author')->first();
        JournalUserRole::create([
            'journal_id' => $journalA->id,
        ...
        // Get roles for Journal B
        $authorRoleB = Role::where('journal_id', $journalB->id)->where('name', 'Author')->first();
        $readerRoleB = Role::where('journal_id', $journalB->id)->where('name', 'Reader')->first();

        // 3. Post to AJAX sync-roles route for Journal B
        $response = $this->actingAs($user)
            ->postJson(route('journal.profile.sync-roles', ['journal' => $journalB->slug]), [
                'role_ids' => [$authorRoleB->id, $readerRoleB->id]
            ]);

        // 4. Assert response is JSON success
        $response->assertStatus(200)
            ->assertJson([
                'success' => true,
                'enrolled' => true,
                'roles' => [$authorRoleB->id, $readerRoleB->id]
            ]);

        // 5. Assert roles are in DB
        $this->assertDatabaseHas('journal_user_roles', [
            'journal_id' => $journalB->id,
            'user_id' => $user->id,
            'role_id' => $authorRoleB->id,
        ...
        // 6. Test unenroll (empty roles)
        $responseUnenroll = $this->actingAs($user)
            ->postJson(route('journal.profile.sync-roles', ['journal' => $journalB->slug]), [
                'role_ids' => []
            ]);

        $responseUnenroll->assertStatus(200)
            ->assertJson([
                'success' => true,
                'enrolled' => false,
                'roles' => []
