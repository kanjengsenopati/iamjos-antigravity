@extends('layouts.master', ['title' => 'Detail Membership', 'main' => 'Membership'])
@section('content')
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="d-flex gap-2 align-items-center mb-2">
                            <a href="{{ route('gym-place.show', $membership->gym_place_id) }}" class="mt-1">
                            ...
                            <h3 class="text-capitalize mb-0">{{ $membership->name }}</h3>
                    ...
                    <div class="card-body v2">
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="tab_detail" role="tabpanel">
                                @if ($membership->type == 'LIMIT_DAY')
                                <span class="badge badge-primary">{{$membership->period}} Hari</span>
                                @elseif ($membership->type == 'LIMIT_DAY_AND_SESSION')
                                <span class="badge badge-primary">{{$membership->period}} Hari</span> 
                                <span class="badge badge-secondary">{{$membership->total_session}} Sesi</span>
                                @else
                                ...
                                <div class="row mb-3">
                                    <div class="col-sm-4">
                                        ...
                                        <p class="text-label">Rp <s class="text-label text-danger">@money($membership->price)</s>
                                            @money($membership->discount_price) <br>
                                            <i>
                                                <small class="text-danger">Promo Berlaku {{$membership->start_date_discount}}
                                                    {{$membership->start_time_discount}} ~ {{$membership->end_date_discount}}
                                                    {{$membership->end_time_discount}}</small>
                                            </i>
                                        </p>
                                        @else
                                        <p class="text-label">Rp @money($membership->price)</p>
                                        @endif
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <div class="col-sm-4">
                                        ...
                                        @foreach ($membership->membership_benefits ?? [] as $membershipBenefit )
                                    ...
                                    <div class="col-sm-4 en-feature">
                                        ...
                                        @foreach ($membership->en_membership_benefits ?? [] as $membershipBenefit )
                                        <p><i class="fa fa-check">&nbsp;</i>{{$membershipBenefit->name}}</p>
                                        @endforeach
                                    </div>
                                    <div class="col-sm-4" style="display: none">
                                        <label class="text-label text-muted">Benefit (cn)</label>
                                        @foreach ($membership->cn_membership_benefits ?? [] as $membershipBenefit )
                                        <p><i class="fa fa-check">&nbsp;</i>{{$membershipBenefit->name}}</p>
                                        @endforeach
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4 en-feature">
                                        ...
                                        <p class="text-label">{{$membership->description}}</p>
                                    </div>
                                    <div class="col-sm-4 en-feature">
                                        <label class="text-label text-muted">Deskripsi (English)</label>
                                        <p class="text-label">{{$membership->description_en}}</p>
                                    </div>
                                    <div class="col-sm-4" style="display: none">
                                        <label class="text-label text-muted">Deskripsi (Chinese)</label>
                                        <p class="text-label">{{$membership->description_cn}}</p>
                                    </div>
                                </div>
                                <div class="card-header mt-6">
                                    ...
                                    <div class="card-toolbar">
                                        ...
                                        <a type="button" class="btn btn-sm btn-primary btn-create"
                                            href="{{ route('membership-product.create',['membership_id' => $membership->id]) }}">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#datatable').DataTable({
                ...
                responsive: true,
                ajax: {
                    url: "{{ route('membership.show', ['membership' => $membership->id, 'data' => 'membership']) }}",
                    type: 'GET',
                },
                language: {
                    "paginate": {
            ...
            var tablePeriod = $('#datatable-price-period').DataTable({
                ...
                responsive: true,
                ajax: {
                    url: "{{ route('membership.show', ['membership' => $membership->id, 'data' => 'price-period']) }}",
                    type: 'GET',
                },
                language: {
                    "paginate": {
