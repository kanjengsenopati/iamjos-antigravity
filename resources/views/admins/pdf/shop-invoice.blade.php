    ...
    <div id="paper">
        ...
        <header>
            <table width="100%" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td align="right">
                        <h1>INVOICE</h1>
                        <p class="text-primary">{{ $data->order_number ?? '' }}</p>
                        <p>{{ $data->translated_status ?? '' }}</p>
                    </td>
                </tr>
            </table>
                ...
                <tr>
                    ...
                    <td width="50%">
                        <table width="100%">
                            <tr></tr>
                            <tr>
                                <td style="white-space: nowrap" width="15%" class="text-muted">Tanggal Pengambilan</td>
                                <td width="2%">:</td>
                                <td>
                                    <strong>{{ \Carbon\Carbon::parse($data->created_at)->locale('id_ID')->isoFormat('D MMMM
                                        YYYY HH:mm') }}</strong>
                                </td>
                            </tr>
                            <tr>
                                <td style="white-space: nowrap" width="15%" class="text-muted">Waktu Pengambilan</td>
                                <td width="2%">:</td>
                                <td>
                                    <strong>{{
                                        \Carbon\Carbon::parse($data->pickup_date)->locale('id_ID')->isoFormat('D
                                        MMMM YYYY HH:mm') }}</strong>
                                </td>
                            </tr>
                        </table>
                    </td>
        ...
        <main class="mt-4">
            <table width="100%" style="border-bottom: 2px solid rgb(232, 232, 232)" cellspacing="0">
                ...
                <tbody>
                    @foreach ($data->shop_order_detail as $detail)
                    <tr>
                        ...
                        <td align="right">Rp{{ number_format($detail->price * $detail->quantity, 0, ',', '.') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <table width="50%" style="margin-left: auto" class="table-tagihan">
                <tr class="text-strong">
                    <td>SUB TOTAL</td>
                    <td align="right">Rp{{ number_format($data->transaction->sub_total, 0, ',', '.') }}</td>
                </tr>

                <tr>
                    <td>Biaya Transaksi</td>
                    <td align="right">Rp{{ number_format($data->transaction->xendit_fee, 0, ',', '.') }}</td>
                </tr>
                @if ($data->discount_product > 0)
                <tr>
                    <td>Diskon Produk</td>
                    <td align="right">-Rp{{ number_format($data->transaction->discount_product, 0, ',', '.') }}</td>
                </tr>
                @endif
                @if ($data->discount_promo > 0)
                <tr>
                    <td class="pb-2">Diskon Promo</td>
                    <td class="pb-2" align="right">-Rp{{ number_format($data->transaction->discount_promo, 0, ',', '.')
                        }}
                    </td>
                </tr>
                @endif
                <tr class="text-strong border-table">
                    <td>TOTAL TAGIHAN</td>
                    <td align="right">Rp{{ number_format($data->transaction->pay_amount, 0, ',', '.') }}</td>
                </tr>
                @if ($data->transaction->discount_promo > 0 || $data->transaction->discount_product > 0)
                <tr>
                    <td class="py-2">
                        <span class="badge badge-success">{{ $data->transaction->discount_promo ? 'Diskon Promo' :
                            'Diskon Produk'
                            }}</span>
                        <br>
                    ...
                    <td class="pt-2" align="right">
                        {{ $data->transaction->discount_promo ? 'Rp' . number_format($data->transaction->discount_promo,
                        0, ',', '.') : 'Rp' .
                        number_format($data->transaction->discount_product, 0, ',', '.') }}
                    </td>
                </tr>
                <tr>
                    <td colspan="2" class="badge-secondary">
                        <i>*Promo yang didapat bisa berubah. Cek
                            <a href="{{ env('APP_URL') }}" target="_blank" class="text-primary text-decoration-none"
            ...
            <table width="100%" class="mt-3" style="border-top: 1.25px solid rgb(232, 232, 232)">
                <tr style="margin-left: auto">
                    <td width="50%">
                        ...
                        <strong>{{ $data->transaction->payment_method->name }}</strong>
                    </td>
                </tr>
            </table>
                ...
                <tr>
                    ...
                    <td width="50%" align="right" style="vertical-align: bottom">
                        <i style="font-family: 'Plus Jakarta Sans', sans-serif !important;">Terakhir diupdate:
                            {{ \Carbon\Carbon::parse($data->updated_at)->locale('id_ID')->isoFormat('D MMMM YYYY HH:mm')
                            }} WIB
                        </i>
                    </td>
                </tr>
