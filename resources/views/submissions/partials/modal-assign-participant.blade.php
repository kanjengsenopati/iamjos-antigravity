                    ...
                    <div class="p-4 border-b border-gray-100 shrink-0 space-y-3">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <select x-model="participantRoleFilter"
                                ...
                                <template x-if="participantModalStage === 'review'">
                                    <option value="Reviewer">{{ $isId ? 'Reviewer' : 'Reviewer' }}</option>
                                </template>
                                <template x-if="participantModalStage === 'copyediting'">
                                    <>
                                        <option value="Copyeditor">{{ $isId ? 'Copyeditor' : 'Copyeditor' }}</option>
                                        <option value="Layout Editor">{{ $isId ? 'Editor Tata Letak' : 'Layout Editor' }}</option>
                                    </>
                                </template>
                                <template x-if="participantModalStage === 'production'">
                                    <>
                                        <option value="Layout Editor">{{ $isId ? 'Editor Tata Letak' : 'Layout Editor' }}</option>
                                        <option value="Proofreader">{{ $isId ? 'Proofreader' : 'Proofreader' }}</option>
                                    </>
                                </template>
                            </select>
                        </div>
