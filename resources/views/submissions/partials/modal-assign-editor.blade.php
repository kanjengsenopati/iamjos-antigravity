        ...
        <div x-show="assignEditorModalOpen" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex-1 overflow-hidden flex flex-col min-h-0">
                <form id="assignEditorForm"
                    action="{{ route('journal.workflow.assign-editor', ['journal' => $journal->slug, 'submission' => $submission->seq_id]) }}"
                    ...
                    <div class="p-4 border-b border-gray-100 flex-shrink-0 space-y-3">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <select x-model="editorRoleFilter"
                                ...
                                <option value="">{{ $isId ? 'Semua Peran Editor' : 'All Editor Roles' }}</option>
                                <option value="Journal Manager">{{ $isId ? 'Manajer Jurnal' : 'Journal Manager' }}</option>
                                <option value="Editor">{{ $isId ? 'Editor' : 'Editor' }}</option>
