...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="sidebarManager()">
        ...
        {{-- Main Layout: Dynamic grid based on form visibility --}}
        <div class="flex flex-col lg:flex-row gap-6">
            {{-- Left Column: Available System Blocks (hidden when form is open) --}}
            <div class="lg:w-[280px] flex-shrink-0 transition-all duration-300"
            ...
            {{-- Center Column: Active Sidebar Blocks --}}
            <div class="flex-1 min-w-0 transition-all duration-300"
                :class="{ 'lg:w-1/2 lg:max-w-[50%]': showAddForm || showEditForm }">
                    ...
                    <div class="p-4 min-h-[400px]" x-ref="activeBlocks" x-init="initSortable($refs.activeBlocks)">
                        @forelse($activeBlocks as $block)
                            <div class="sidebar-block bg-white border border-gray-200 rounded-xl p-4 mb-3 hover:shadow-md transition-all cursor-move"
                                data-id="{{ $block->id }}"
                                :class="{ 'ring-2 ring-primary-300 bg-primary-50/30': showEditForm && editingBlock.id == '{{ $block->id }}' }">
            ...
            {{-- Right Column: Inline Add Form (50% width) --}}
            <div class="lg:w-1/2 flex-shrink-0 transition-all duration-300"
                x-show="showAddForm" x-cloak
            ...
            {{-- Right Column: Inline Edit Form (50% width) --}}
            <div class="lg:w-1/2 flex-shrink-0 transition-all duration-300"
                x-show="showEditForm" x-cloak
                x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-x-8"
                x-transition:enter-end="opacity-100 translate-x-0"
                x-transition:leave="transition ease-in duration-200"
    ...
    <script>
        function sidebarManager() {
            return {
                ...
                // Destroy TinyMCE instances safely
                ...
                openAddForm() {
                    ...
                    // Small delay to ensure form panel is rendered
                    setTimeout(() => {
                ...
                closeAddForm() {
                    this.destroyTinyMCE('new-block-teaser');
                    this.destroyTinyMCE('new-block-content');
                ...
                editBlock(block) {
                    ...
                    // Close existing edit form TinyMCE instances
                    if (this.showEditForm) {
                        this.destroyTinyMCE('edit-block-teaser');
                        this.destroyTinyMCE('edit-block-content');
                    }
