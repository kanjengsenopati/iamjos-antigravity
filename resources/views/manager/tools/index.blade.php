...
@section('title', ($isId ? 'Alat' : 'Tools') . ' - ' . $journal->name)
...
@section('content')
    @php
        $toolTranslations = [
            'import_oai' => [
                'title' => 'Impor OAI-PMH',
                'desc' => 'Impor artikel dan metadata dari endpoint OAI-PMH eksternal (misal: OJS 3.x).'
            ],
            'scholar' => [
                'title' => 'Pemantau Google Scholar',
                'desc' => 'Pantau status pengindeksan artikel Anda di Google Scholar dan dapatkan pemberitahuan jika ada perubahan.'
            ],
            'native' => [
                'title' => 'Plugin XML Native',
                'desc' => 'Impor dan ekspor artikel serta terbitan dalam format XML native IAMJOS untuk pencadangan atau migrasi.'
            ],
            'users' => [
                'title' => 'Plugin XML Pengguna',
                'desc' => 'Impor dan ekspor akun pengguna serta peran dalam format XML untuk pengelolaan massal.'
            ],
            'copernicus' => [
                'title' => 'Eksportir XML ICI',
                'desc' => 'Ekspor metadata artikel dan terbitan dalam format yang diwajibkan oleh Index Copernicus (ICI) World of Journals.'
            ],
            'crossref' => [
                'title' => 'Ekspor XML CrossRef',
                'desc' => 'Ekspor metadata artikel dalam format XML CrossRef untuk pendaftaran DOI dan tautan sitasi.'
            ],
            'pubmed' => [
                'title' => 'Ekspor XML PubMed',
                'desc' => 'Ekspor metadata artikel dalam format XML PubMed untuk pengindeksan di MEDLINE/PubMed.'
            ],
            'doaj' => [
                'title' => 'Plugin Ekspor DOAJ',
                'desc' => 'Ekspor metadata jurnal untuk pengindeksan Directory of Open Access Journals (DOAJ).'
            ],
            'datacite' => [
                'title' => 'Ekspor/Registrasi DataCite',
                'desc' => 'Ekspor atau daftarkan metadata artikel dan berkas tambahan dalam format DataCite untuk DOI.'
            ],
            'quicksubmit' => [
                'title' => 'Plugin Quick Submit',
    ...
    <div x-data="toolsPage()" class="space-y-6">
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">
                    {{ $isId ? 'Utilitas untuk impor, ekspor, dan pemeliharaan sistem.' : 'Utilities for import, export, and system maintenance.' }}
                </p>
            </div>
            ...
            <div x-show="activeTab === 'import'" x-transition class="w-full md:w-auto">
                <div class="relative">
                    <input type="text" x-model="search" placeholder="{{ $isId ? 'Cari alat...' : 'Search tools...' }}"
        ...
        <div class="border-b border-slate-200 mb-6">
            <nav class="flex space-x-8 overflow-x-auto no-scrollbar" aria-label="Tabs">
                <button type="button" @click="activeTab = 'import'"
                    :class="activeTab === 'import' ? 'border-primary-600 text-primary-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'"
                    ...
                    {{ $isId ? 'Impor / Ekspor' : 'Import / Export' }}
                </button>
                <button type="button" @click="activeTab = 'permissions'"
                    :class="activeTab === 'permissions' ? 'border-primary-600 text-primary-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'"
                    class="flex-shrink-0 py-4 px-1 text-sm font-medium border-b-2 transition-all whitespace-nowrap flex items-center gap-2 cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div x-show="activeTab === 'import'" x-transition:enter="transition ease-out duration-200"
            ...
            <div x-show="!hasVisibleTools()" x-cloak class="text-center py-16 text-slate-400">
                ...
                <p class="font-medium">
                    {{ $isId ? 'Tidak ada alat yang cocok dengan pencarian Anda.' : 'No tools found matching your search.' }}
                </p>
                <p class="text-sm mt-1">{{ $isId ? 'Coba kata kunci lain.' : 'Try a different keyword.' }}</p>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'permissions'" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <div class="bg-white rounded-xl border border-red-200 overflow-hidden">
                ...
                <div class="bg-red-50 px-6 py-4 border-b border-red-100 flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-lg text-red-800">{{ $isId ? 'Reset Hak Akses Artikel' : 'Reset Article Permissions' }}</h3>
                        <p class="text-xs text-red-600">{{ $isId ? 'Zona Bahaya - Tindakan ini tidak dapat dibatalkan' : 'Danger Zone - This action cannot be undone' }}</p>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <p class="text-slate-700 mb-4 leading-relaxed">
                        {{ $isId ? 'Hapus pernyataan hak cipta dan informasi lisensi untuk setiap artikel yang diterbitkan, mengembalikannya ke pengaturan default jurnal saat ini. Ini akan menghapus secara permanen semua informasi hak cipta dan lisensi sebelumnya yang melekat pada artikel.' : 'Remove the copyright statement and license information for every published article, reverting them to the journal\'s current default settings. This will permanently remove all prior copyright and licensing information attached to articles.' }}
                    </p>
                    ...
                    <div class="bg-amber-50 border-l-4 border-amber-400 rounded-r-xl p-4 mb-6">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-amber-800 mb-1">{{ $isId ? 'Peringatan Hukum' : 'Legal Warning' }}</p>
                                <p class="text-sm text-amber-700">
                                    {{ $isId ? 'Dalam beberapa kasus, Anda mungkin tidak diizinkan secara hukum untuk melisensikan kembali karya yang telah diterbitkan di bawah lisensi yang berbeda. Harap berhati-hati saat menggunakan alat ini dan berkonsultasi dengan ahli hukum jika Anda tidak yakin hak apa yang Anda miliki atas artikel yang diterbitkan di jurnal Anda.' : 'In some cases, you may not be legally permitted to re-license work that has been published under a different license. Please take caution when using this tool and consult legal expertise if you are unsure what rights you hold over the articles published in your journal.' }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-data="{ confirmReset: false }">
                        <button @click="confirmReset = true"
                            ...
                            {{ $isId ? 'Reset Hak Akses Artikel' : 'Reset Article Permissions' }}
                        </button>
                        ...
                        <div x-show="confirmReset" x-cloak
                            ...
                            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden"
                                ...
                                <div class="p-6">
                                    ...
                                    <h3 class="text-lg font-bold text-slate-900 text-center mb-2">{{ $isId ? 'Apakah Anda benar-benar yakin?' : 'Are you absolutely sure?' }}</h3>
                                    <p class="text-sm text-slate-500 text-center mb-6">
                                        {{ $isId ? 'Tindakan ini tidak dapat dibatalkan. Semua lisensi kustom pada artikel yang diterbitkan akan ditimpa dengan pengaturan default jurnal saat ini.' : 'This action cannot be undone. All custom licenses on existing published articles will be overwritten by the current journal default settings.' }}
                                    </p>

                                    <div class="flex gap-3">
                                        <button @click="confirmReset = false"
                                            class="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 bg-white hover:bg-slate-50 transition-colors cursor-pointer">
                                            {{ $isId ? 'Batal' : 'Cancel' }}
                                        </button>
                                        <form
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="w-full px-4 py-2.5 rounded-xl text-sm font-medium text-white bg-red-600 hover:bg-red-700 transition-colors cursor-pointer">
                                                {{ $isId ? 'Ya, Reset Semua' : 'Yes, Reset Everything' }}
                                            </button>
                                        </form>
                                    </div>
                                </div>
