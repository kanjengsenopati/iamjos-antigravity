...
@section('content')
    <div x-data="userXmlPlugin()" class="space-y-6">
        ...
        <div class="border-b border-slate-200 mb-6">
            <nav class="flex space-x-8 overflow-x-auto no-scrollbar" aria-label="Tabs">
                <button type="button" @click="tab = 'import'"
                    ...
                    {{ $isId ? 'Impor Pengguna' : 'Import Users' }}
                </button>
                <button type="button" @click="tab = 'export'"
                    :class="tab === 'export' ? 'border-primary-600 text-primary-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'"
                    class="flex-shrink-0 py-4 px-1 text-sm font-medium border-b-2 transition-all whitespace-nowrap flex items-center gap-2 cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    {{ $isId ? 'Ekspor Pengguna' : 'Export Users' }}
                </button>
            </nav>
        </div>
        ...
        <div x-show="tab === 'import'" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="max-w-2xl">
                <h3 class="font-bold text-lg text-slate-800 mb-2">{{ $isId ? 'Impor Akun Pengguna' : 'Import User Accounts' }}</h3>
                <p class="text-sm text-slate-500 mb-6">
                    {{ $isId ? 'Unggah berkas XML yang kompatibel untuk mengimpor pengguna dan penetapan peran mereka. Akun akan ditautkan berdasarkan email.' : 'Upload a compatible XML file to import users and their role assignments. Accounts will be linked by email.' }}
                </p>
                ...
                <form action="{{ route('journal.settings.tools.users.import', ['journal' => $journal->slug]) }}"
                    ...
                    <div class="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center hover:border-indigo-400 transition-colors"
                        ...
                        <label for="xml_file" class="cursor-pointer">
                            ...
                            <p class="text-slate-600 mb-2"><span class="font-medium">{{ $isId ? 'Klik untuk memilih' : 'Click to upload' }}</span> {{ $isId ? 'atau seret dan lepas berkas' : 'or drag and drop' }}</p>
                            <p class="text-xs text-slate-400">{{ $isId ? 'Hanya berkas XML (maks. 10MB)' : 'XML files only (max. 10MB)' }}</p>
                        </label>
                    ...
                    <button type="submit" class="w-full px-5 py-3 rounded-xl text-sm font-medium text-white shadow-lg shadow-indigo-500/25 transition-all flex items-center justify-center gap-2"
                        ...
                        {{ $isId ? 'Proses Impor' : 'Process Import' }}
                    </button>
                </form>
            </div>
        </div>
        ...
        <div x-show="tab === 'export'" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <form action="{{ route('journal.settings.tools.users.export', ['journal' => $journal->slug]) }}" method="POST">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h3 class="font-bold text-lg text-slate-800">{{ $isId ? 'Pilih Pengguna untuk Diekspor' : 'Select Users to Export' }}</h3>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead class="bg-slate-50 text-slate-600 border-b border-slate-200">
                            <tr>
                                ...
                                <th class="p-4">{{ $isId ? 'Nama' : 'Name' }}</th>
                                <th class="p-4">Email</th>
                                <th class="p-4">{{ $isId ? 'Peran' : 'Roles' }}</th>
                            </tr>
                        </thead>
                ...
                <div class="mt-6 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div class="text-xs text-slate-500 font-bold">
                        {{ $isId ? 'Menampilkan' : 'Showing' }} {{ $users->firstItem() ?? 0 }} {{ $isId ? 'sampai' : 'to' }} {{ $users->lastItem() ?? 0 }} {{ $isId ? 'dari' : 'of' }} {{ $users->total() }} {{ $isId ? 'pengguna' : 'users' }}
                    </div>
