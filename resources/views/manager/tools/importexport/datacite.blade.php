...
@section('title', 'DataCite Export Plugin - ' . $journal->name)

@section('content')
    <div x-data="datacitePlugin()" class="space-y-6">
            ...
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-2xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text">
                        DataCite Export Plugin
                    </h1>
                </div>
                <p class="text-sm text-slate-500 mt-1 ml-8">Export or register article and supplementary file metadata in DataCite format for DOIs.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden p-6">
            <form action="{{ route('journal.settings.tools.datacite.export', ['journal' => $journal->slug]) }}" method="POST">
                @csrf
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button type="submit" class="px-5 py-2.5 rounded-xl text-sm font-medium text-white shadow-lg shadow-rose-500/25 transition-all"
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                ...
                                <td class="p-4 text-slate-600">
                                    {{ $submission->currentPublication->doi ?? '-' }}
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
