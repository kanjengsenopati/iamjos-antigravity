...
@section('content')
    <div x-data="nativeXmlPlugin()" class="space-y-6">
        ...
        <div class="border-b border-slate-200 mb-6">
            <nav class="flex space-x-8 overflow-x-auto no-scrollbar" aria-label="Tabs">
                <button type="button" @click="tab = 'import'"
                    :class="tab === 'import' ? 'border-primary-600 text-primary-600 font-bold' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'"
        ...
        <div x-show="tab === 'import'" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="w-full">
                <h3 class="font-bold text-lg text-slate-800 mb-2">{{ $isId ? 'Unggah Berkas XML' : 'Upload XML File' }}</h3>
                <p class="text-sm text-slate-500 mb-6">
                    {{ $isId ? 'Unggah artikel atau terbitan dalam format XML Native IAMJOS. Sistem akan secara otomatis memproses dan mengimpor semua data termasuk penulis dan metadata.' : 'Upload articles or issues in IAMJOS Native XML format. The system will automatically process and import all data including authors and metadata.' }}
                </p>
                ...
                <form action="{{ route('journal.settings.tools.native.import', ['journal' => $journal->slug]) }}"
                    ...
                    <div class="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center hover:border-indigo-400 transition-colors"
                        ...
                        <p class="text-slate-600 mb-2">
                            <span class="font-medium">{{ $isId ? 'Letakkan berkas XML Anda di sini' : 'Drop your XML file here' }}</span> {{ $isId ? 'atau klik untuk memilih' : 'or click to browse' }}
                        </p>
                        <p class="text-xs text-slate-400 mb-4">{{ $isId ? 'Ukuran berkas maksimum: 10MB' : 'Maximum file size: 10MB' }}</p>
        ...
        <div x-show="tab === 'articles'" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <form action="{{ route('journal.settings.tools.native.export.articles', ['journal' => $journal->slug]) }}"
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div>
                        <h3 class="font-bold text-lg text-slate-800">{{ $isId ? 'Pilih Artikel untuk Diekspor' : 'Select Articles to Export' }}</h3>
                        <p class="text-sm text-slate-500">{{ $isId ? 'Pilih artikel tertentu untuk dimasukkan ke dalam ekspor.' : 'Choose specific articles to include in the export.' }}</p>
                    </div>
                    <button type="submit"
                        ...
                        {{ $isId ? 'Ekspor Artikel Terpilih' : 'Export Selected Articles' }}
                    </button>
                </div>
                            ...
                            <table class="w-full text-sm text-left">
                                <thead class="bg-slate-50 text-slate-600 border-b border-slate-200 sticky top-0 z-10">
                                    <tr>
                                        ...
                                        <th class="p-4 font-semibold">{{ $isId ? 'Status' : 'Status' }}</th>
                                        ...
                                        <tr class="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td class="p-4">
                                                <span
                                                    ...
                                                    {{ $isId ? ($submission->status === 'published' ? 'Diterbitkan' : ($submission->status === 'accepted' ? 'Diterima' : 'Draf')) : ucwords(str_replace('_', ' ', $submission->status)) }}
                                                </span>
                                            </td>
                ...
                @else
                    <div class="text-center py-16 text-slate-400">
                        ...
                        <p class="font-medium">{{ $isId ? 'Tidak ada artikel ditemukan' : 'No articles found' }}</p>
                        <p class="text-sm mt-1">{{ $isId ? 'Kirim artikel untuk mengekspornya.' : 'Submit articles to export them.' }}</p>
                    </div>
                @endif
            </form>
        </div>
        ...
        <div x-show="tab === 'issues'" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <form action="{{ route('journal.settings.tools.native.export.issues', ['journal' => $journal->slug]) }}"
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div>
                        <h3 class="font-bold text-lg text-slate-800">{{ $isId ? 'Pilih Terbitan untuk Diekspor' : 'Select Issues to Export' }}</h3>
                        <p class="text-sm text-slate-500">{{ $isId ? 'Terbitan akan diekspor beserta seluruh artikel di dalamnya.' : 'Issues will be exported with all their contained articles.' }}</p>
                    </div>
                    <button type="submit"
                        ...
                        {{ $isId ? 'Ekspor Terbitan Terpilih' : 'Export Selected Issues' }}
                    </button>
                </div>
                                            ...
                                            <td class="p-4">
                                                ...
                                                @else
                                                    <span
                                                        ...
                                                        {{ $isId ? 'Belum Diterbitkan' : 'Unpublished' }}
                                                    </span>
                                                @endif
                                            </td>
                                            <td class="p-4">
                ...
                @else
                    <div class="text-center py-16 text-slate-400">
                        ...
                        <p class="font-medium">{{ $isId ? 'Tidak ada terbitan ditemukan' : 'No issues found' }}</p>
                        <p class="text-sm mt-1">{{ $isId ? 'Buat terbitan untuk mengekspornya.' : 'Create issues to export them.' }}</p>
                    </div>
                @endif
            </form>
        </div>
