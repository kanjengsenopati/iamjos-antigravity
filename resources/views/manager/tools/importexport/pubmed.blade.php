...
@section('title', 'PubMed XML Export - ' . $journal->name)

@section('content')
    <div x-data="pubmedPlugin()" class="space-y-6">
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1 ml-8">Export article metadata in PubMed XML format for indexing in MEDLINE/PubMed.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden p-6">
            <form action="{{ route('journal.settings.tools.pubmed.export', ['journal' => $journal->slug]) }}" method="POST">
                @csrf
