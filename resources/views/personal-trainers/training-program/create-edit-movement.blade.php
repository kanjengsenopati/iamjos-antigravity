@extends('layouts.pt-master', ['title' => request()->routeIs('trainer.movement.create') ? 'Tambah Movement' : 'Edit Movement'])
                ...
                <div class="card h-lg-100">
                    <div class="card-header">
                        <h3 class="card-title align-items-start flex-column">
                            {{ request()->routeIs('trainer.movement.create') ? 'Tambah Movement' : 'Edit Movement' }}
                        </h3>
                    </div>
                    <div class="card-body pt-5">
                        <form action="{{ request()->routeIs('trainer.movement.create') ? route('trainer.movement.store') : route('trainer.movement.update', @$movement->id) }}"
                            method="POST" enctype="multipart/form-data">
                            @csrf
                            @if(!request()->routeIs('trainer.movement.create'))
                            @method('PUT')
                            @endif
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('trainer.movement.index') }}">
