...
<section class="py-16 md:py-24 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-12">
            <div>
                <span class="inline-flex items-center px-3 py-1 text-sm font-medium text-green-600 bg-green-100 rounded-full mb-4">
        ...
        @if($articles->isNotEmpty())
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-{{ $columns }} gap-6">
                @foreach($articles->take($limit) as $article)
                    <article class="bg-white rounded-xl shadow-sm hover:shadow-lg border border-gray-100 overflow-hidden transition-all duration-300 group">
                        {{-- Article Header --}}
                        <div class="p-6 pb-4">
                            {{-- Journal Badge --}}
                            ...
                            {{-- Title --}}
                            <h3 class="font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-blue-600 transition-colors">
                            ...
                            @if($showAuthors && $article->authors && $article->authors->isNotEmpty())
                                <div class="flex items-center gap-2 text-sm text-gray-500 mb-3">
                                    ...
                                    <span class="line-clamp-1">
                                        {{ $article->authors->pluck('full_name')->take(3)->join(', ') }}
                                        @if($article->authors->count() > 3)
                                            <span class="text-gray-400">+{{ $article->authors->count() - 3 }} more</span>
                                        @endif
                                    </span>
                                </div>
                            @endif
