...
<div class="text-sm text-slate-600">
    @auth
        <p class="mb-3">{{ __('You are logged in as') }} <strong>{{ auth()->user()->name }}</strong>.</p>
    ...
    @else
        <form action="{{ route('journal.login', $journal->slug) }}" method="POST" class="space-y-3">
            @csrf
            <div>
                <label for="block-login-email" class="sr-only">{{ __('Email') }}</label>
                <input type="email" name="email" id="block-login-email" required placeholder="{{ __('Email') }}" class="w-full border-slate-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
            <div>
                <label for="block-login-password" class="sr-only">{{ __('Password') }}</label>
                <input type="password" name="password" id="block-login-password" required placeholder="{{ __('Password') }}" class="w-full border-slate-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
        ...
        <div class="mt-3 text-xs text-center flex items-center justify-center">
            <a href="{{ route('journal.register', $journal->slug) }}" class="text-indigo-600 hover:underline">{{ __('Register') }}</a>
            <span class="mx-2 text-slate-300">|</span>
            <a href="{{ route('forgot-password') }}" class="text-indigo-600 hover:underline">{{ __('Forgot Password') }}</a>
        </div>
    @endauth
</div>
