    ...
    <aside
        ...
        @if ($userJournals->count() > 0)
            <div class="px-4 py-3 border-b border-slate-700/50 relative" x-data="{ journalOpen: false, search: '' }" @click.outside="journalOpen = false">
                ...
                <!-- Journal Dropdown (Legacy Dark Theme Flyout) -->
                <div x-show="journalOpen" x-cloak
                    ...
                    class="absolute left-full ml-3 top-0 min-w-max max-w-md z-[100] bg-slate-800 rounded-xl shadow-2xl border border-slate-700 flex flex-col before:content-[''] before:absolute before:top-5 before:-left-2 before:border-y-8 before:border-y-transparent before:border-r-8 before:border-r-slate-800">
                        ...
                        <div class="max-h-[60vh] overflow-y-auto custom-scrollbar flex flex-col space-y-1 px-1 pr-2">
                            @foreach ($userJournals as $j)
                                <a href="{{ route('journal.submissions.index', ['journal' => $j->path]) }}"
                                    ...
                                    <!-- Abbr Badge -->
                                    <div class="w-8 h-8 rounded bg-slate-900 flex items-center justify-center text-xs font-bold text-slate-400 border border-slate-700 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-500 transition-colors shrink-0">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('admin.journals.index') }}"
                ...
                <span>{{ $isId ? 'Kelola Jurnal' : 'Hosted Journals' }}</span>
            </a>
            ...
            <a href="{{ route('admin.site.settings.form') }}"
                ...
                <span>{{ $isId ? 'Pengaturan Situs' : 'Site Settings' }}</span>
            </a>
            ...
            <a href="{{ route('admin.system-settings.index') }}"
                ...
                <span>{{ $isId ? 'Pengaturan Sistem' : 'System Settings' }}</span>
            </a>
            ...
            <a href="{{ route('admin.public-page.index') }}"
                ...
                <span>{{ $isId ? 'Halaman Publik' : 'Public Page' }}</span>
            </a>
            <!-- System & Maintenance -->
            <a href="{{ route('admin.site.system-info') }}"
                ...
                <span>{{ $isId ? 'Sistem & Pemeliharaan' : 'System & Maintenance' }}</span>
            </a>

            <!-- OJS Migration -->
            <a href="{{ route('admin.tools.migration.index') }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200
                {{ request()->routeIs('admin.tools.migration.*') ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                </svg>
                <span>{{ $isId ? 'Migrasi SQL OJS' : 'OJS SQL Migration' }}</span>
            </a>
            ...
            <div class="pt-6 mt-6 border-t border-slate-700/50">
                ...
                <a href="{{ route('portal.home') }}"
                    ...
                    <span>{{ $isId ? 'Kembali ke Halaman Publik' : 'Back to Public Page' }}</span>
                </a>
            </div>
        </nav>
        ...
        <div class="p-4 border-t border-slate-700/50" x-data="{ userMenuOpen: false }"> <button
            ...
            <div x-show="userMenuOpen" @click.away="userMenuOpen = false" x-cloak x-transition
                class="mt-2 py-1 bg-slate-800 rounded-lg shadow-lg"> <a
                    ...
                    </svg> {{ $isId ? 'Pengaturan Profil' : 'Profile Settings' }} </a>
                <form action="{{ route('logout') }}" method="POST"> @csrf <button type="submit"
                        ...
                        </svg> {{ $isId ? 'Keluar' : 'Logout' }} </button> </form>
            </div>
        </div>
    ...
    <main class="main-content min-h-screen">
        ...
        <header
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="text-sm text-gray-500 hidden lg:block">{{ $isId ? 'Administrasi Situs' : 'Site Administration' }}</span>
            </div>
                    ...
                    <div x-show="profileOpen" @click.away="profileOpen = false" x-cloak x-transition
                        ...
                        <form action="{{ route('logout') }}" method="POST">
                            ...
                            <button type="submit"
                                ...
                                <i class="fa-solid fa-right-from-bracket text-red-400"></i> {{ $isId ? 'Keluar' : 'Logout' }}
                            </button>
                        </form>
                    </div>
                </div>
