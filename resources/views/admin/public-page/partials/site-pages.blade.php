    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-3">
                ...
                {{ $isId ? 'Halaman Situs' : 'Site Pages' }}
            </h2>
            <p class="text-slate-600 mt-1">{{ $isId ? 'Buat dan kelola halaman kustom untuk portal Anda' : 'Create and manage custom pages for your portal' }}</p>
        </div>
        <a href="{{ route('admin.site-pages.create') }}" 
           class="group inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl hover:from-emerald-600 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 cursor-pointer">
            ...
            <span class="font-semibold">{{ $isId ? 'Buat Halaman Baru' : 'Create New Page' }}</span>
        </a>
    </div>
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-emerald-100 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-600">{{ $isId ? 'Total Halaman' : 'Total Pages' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-green-100 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-600">{{ $isId ? 'Diterbitkan' : 'Published' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-100 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-600">{{ $isId ? 'Draf' : 'Drafts' }}</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="px-6 py-4 bg-slate-50/50 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.site-pages.edit', $page) }}" 
                               class="inline-flex items-center gap-1 px-3 py-1.5 bg-white text-emerald-600 rounded-lg hover:bg-emerald-50 transition-colors text-sm font-medium border border-emerald-200 hover:border-emerald-300 cursor-pointer">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @if($page->published)
                                <a href="{{ route('site.page', $page->slug) }}" 
                                   target="_blank"
                                   class="inline-flex items-center gap-1 px-3 py-1.5 bg-white text-blue-600 rounded-lg hover:bg-blue-50 transition-colors text-sm font-medium border border-blue-200 hover:border-blue-300 cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <form method="POST" action="{{ route('admin.site-pages.destroy', $page) }}" 
                            ...
                            @method('DELETE')
                            <button type="submit" 
                                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-white text-red-600 rounded-lg hover:bg-red-50 transition-colors text-sm font-medium border border-red-200 hover:border-red-300 cursor-pointer">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @else
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center">
            <div class="max-w-md mx-auto">
                ...
                <h3 class="text-xl font-bold text-slate-800 mb-2">{{ $isId ? 'Belum Ada Halaman' : 'No Pages Yet' }}</h3>
                <p class="text-slate-600 mb-6">{{ $isId ? 'Mulailah dengan membuat halaman kustom pertama untuk portal Anda.' : 'Get started by creating your first custom page for your portal.' }}</p>
                <a href="{{ route('admin.site-pages.create') }}" 
                   class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl hover:from-emerald-600 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-xl font-semibold cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    {{ $isId ? 'Buat Halaman Pertama Anda' : 'Create Your First Page' }}
                </a>
            </div>
        </div>
    @endif
