    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-3">
                ...
                {{ $isId ? 'Navigasi Situs' : 'Site Navigation' }}
            </h2>
            <p class="text-slate-600 mt-1">{{ $isId ? 'Konfigurasi menu navigasi dan item menu' : 'Configure navigation menus and menu items' }}</p>
        </div>
    </div>
    ...
    @if($menus->count() > 0)
                    ...
                    <div class="px-6 py-4 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-100">
                        <div class="flex items-center justify-between">
                            ...
                            <button onclick="openAddItemModal({{ $menu->id }})"
                                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-orange-500 to-amber-600 text-white text-sm rounded-lg hover:from-orange-600 hover:to-amber-700 transition-all duration-200 shadow-sm font-medium cursor-pointer">
                        ...
                        @if($menu->items->count() > 0)
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onclick="editMenuItem({{ $item->id }})"
                                                        class="p-2 text-orange-600 hover:bg-orange-100 rounded-lg transition-colors cursor-pointer">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <form method="POST" action="{{ route('admin.site-navigation.items.destroy', $item) }}" 
                                                      class="inline"
                                                      onsubmit="return confirm('{{ $isId ? 'Hapus item menu ini?' : 'Delete this menu item?' }}')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                            ...
                            <div class="mt-4 pt-4 border-t border-slate-100 text-center">
                                <p class="text-sm text-slate-600">
                                    <span class="font-semibold text-orange-600">{{ $menu->items->count() }}</span> {{ $isId ? 'item menu' : 'menu items' }}
                                </p>
                            </div>
                        @else
                            <!-- Empty Menu State -->
                            <div class="text-center py-8">
                                ...
                                <p class="text-slate-500 text-sm mb-4">{{ $isId ? 'Belum ada item menu' : 'No menu items yet' }}</p>
                                <button onclick="openAddItemModal({{ $menu->id }})"
                                        class="inline-flex items-center gap-2 px-4 py-2 bg-orange-50 text-orange-600 rounded-lg hover:bg-orange-100 transition-colors text-sm font-medium cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                    </svg>
                                    {{ $isId ? 'Tambah Item Pertama' : 'Add First Item' }}
                                </button>
                            </div>
                        @endif
                    </div>
        ...
        <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl border border-orange-100 p-6">
            <h3 class="font-bold text-slate-800 mb-4 flex items-center gap-2">
                ...
                {{ $isId ? 'Rute Tersedia' : 'Available Routes' }}
            </h3>
    ...
    @else
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center">
            <div class="max-w-md mx-auto">
                ...
                <h3 class="text-xl font-bold text-slate-800 mb-2">{{ $isId ? 'Tidak Ada Menu Navigasi' : 'No Navigation Menus' }}</h3>
                <p class="text-slate-600">{{ $isId ? 'Menu navigasi akan muncul di sini setelah dikonfigurasi.' : 'Navigation menus will appear here once they are configured.' }}</p>
            </div>
        </div>
    @endif
</div>
