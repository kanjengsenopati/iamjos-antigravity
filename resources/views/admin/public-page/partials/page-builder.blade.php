    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-3">
                ...
                {{ $isId ? 'Pembuat Halaman' : 'Page Builder' }}
            </h2>
            <p class="text-slate-600 mt-1">{{ $isId ? 'Sesuaikan tampilan portal Anda dengan blok konten' : "Customize your portal's appearance with content blocks" }}</p>
        </div>
    </div>
    ...
    @if($blocks->count() > 0)
        ...
        <div class="flex flex-wrap gap-2">
            <button @click="activeCategory = 'all'"
                    ...
                    class="px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 border border-purple-200 cursor-pointer">
                {{ $isId ? 'Semua Blok' : 'All Blocks' }}
            </button>
            @foreach($blocksByCategory->keys() as $category)
                <button @click="activeCategory = '{{ $category }}'"
                        ...
                        class="px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 border border-purple-200 capitalize cursor-pointer">
                    {{ $category }}
                </button>
            @endforeach
        </div>
                ...
                <div x-show="activeCategory === 'all' || activeCategory === '{{ $category }}'"
                    ...
                    <div class="mb-4">
                        <h3 class="text-lg font-bold text-slate-800 capitalize flex items-center gap-2">
                            ...
                            {{ $category }}
                            <span class="text-sm font-normal text-slate-500">({{ $categoryBlocks->count() }} {{ $isId ? 'blok' : 'blocks' }})</span>
                        </h3>
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($categoryBlocks as $block)
                            <div class="group bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200 hover:border-purple-300 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden">
                                <div class="p-6">
                                    ...
                                    <div class="flex items-center gap-4 text-xs text-slate-500 mb-4">
                                        ...
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full {{ $block->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700' }}">
                                            {{ $block->is_active ? ($isId ? '● Aktif' : '● Active') : ($isId ? '○ Nonaktif' : '○ Inactive') }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2 pt-4 border-t border-slate-100">
                                        <a href="{{ route('admin.site.appearance.edit', $block) }}" 
                                           class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100 transition-colors text-sm font-medium cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                            {{ $isId ? 'Edit Konten' : 'Edit Content' }}
                                        </a>
                                        <a href="{{ route('admin.site.appearance.config', $block) }}" 
                                           class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-slate-50 text-slate-600 rounded-lg hover:bg-slate-100 transition-colors text-sm font-medium cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @else
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center">
            <div class="max-w-md mx-auto">
                ...
                <h3 class="text-xl font-bold text-slate-800 mb-2">{{ $isId ? 'Tidak Ada Blok Konten' : 'No Content Blocks' }}</h3>
                <p class="text-slate-600">{{ $isId ? 'Blok konten akan muncul di sini setelah dikonfigurasi.' : 'Content blocks will appear here once they are configured.' }}</p>
            </div>
        </div>
    @endif
</div>
