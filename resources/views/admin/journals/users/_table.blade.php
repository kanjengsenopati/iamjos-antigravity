    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col"
                        class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $isId ? 'Nama Depan' : 'Given Name' }}</th>
                    <th scope="col"
                        class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $isId ? 'Nama Belakang' : 'Family Name' }}</th>
                    <th scope="col"
                    ...
                    <tr class="hover:bg-gray-50/70 transition-colors cursor-pointer"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                @if($user->disabled)
                                    <span class="ml-2 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider bg-red-100 text-red-700 rounded-md">{{ $isId ? 'Nonaktif' : 'Disabled' }}</span>
                                @endif
                            </div>
                        </td>
                            ...
                            <div class="flex flex-wrap gap-2.5 text-xs font-semibold items-center">
                                @if ($isSuperAdmin)
                                    <span class="text-xs text-purple-600 font-medium px-2 py-0.5 bg-purple-50 rounded-lg border border-purple-200/50">
                                        <i class="fa-solid fa-shield-halved mr-1"></i> {{ $isId ? 'Akses Penuh' : 'Full Access' }}
                                    </span>
                                @else
                                    <!-- Email -->
                                    <button type="button"
                                        ...
                                        <i class="fa-solid fa-envelope text-[11px]"></i> {{ $isId ? 'Surel' : 'Email' }}
                                    </button>
                                    ...
                                    <a href="{{ route($routePrefix . '.edit', ['journal' => $journal->slug, 'user' => $user->id]) }}"
                                        ...
                                        <i class="fa-solid fa-user-pen text-[11px]"></i> {{ $isId ? 'Ubah Pengguna' : 'Edit User' }}
                                    </a>
                                    ...
                                    <form
                                        ...
                                        @if ($user->disabled)
                                            <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border transition-all duration-150 shadow-sm text-emerald-600 bg-emerald-50 border-emerald-200/60 hover:bg-emerald-100/80 focus:outline-none focus:ring-2 focus:ring-emerald-500/20">
                                                <i class="fa-solid fa-circle-check text-[11px]"></i> {{ $isId ? 'Aktifkan' : 'Enable' }}
                                            </button>
                                        @else
                                            <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border transition-all duration-150 shadow-sm text-red-600 bg-red-50 border-red-200/60 hover:bg-red-100/80 focus:outline-none focus:ring-2 focus:ring-red-500/20">
                                                <i class="fa-solid fa-ban text-[11px]"></i> {{ $isId ? 'Nonaktifkan' : 'Disable' }}
                                            </button>
                                        @endif
                                    </form>
                                    ...
                                    <form
                                        ...
                                        method="POST" class="inline" @click.stop
                                        onsubmit="return confirm('{{ $isId ? 'Hapus pengguna ini dari ' . $journal->name . '? Mereka tidak akan lagi memiliki akses ke jurnal ini.' : 'Remove this user from ' . $journal->name . '? They will no longer have access to this journal.' }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border transition-all duration-150 shadow-sm text-red-600 bg-red-50 border-red-200/60 hover:bg-red-100/80 focus:outline-none focus:ring-2 focus:ring-red-500/20">
                                            <i class="fa-solid fa-trash-can text-[11px]"></i> {{ $isId ? 'Hapus' : 'Remove' }}
                                        </button>
                                    </form>
                                    ...
                                    <form
                                        ...
                                        <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border transition-all duration-150 shadow-sm text-violet-600 bg-violet-50 border-violet-200/60 hover:bg-violet-100/80 focus:outline-none focus:ring-2 focus:ring-violet-500/20">
                                            <i class="fa-solid fa-user-secret text-[11px]"></i> {{ $isId ? 'Masuk Sebagai' : 'Login As' }}
                                        </button>
                                    </form>
                                    ...
                                    <a href="{{ route($routePrefix . '.merge', ['journal' => $journal->slug, 'user' => $user->id]) }}"
                                        ...
                                        <i class="fa-solid fa-code-merge text-[11px]"></i> {{ $isId ? 'Gabungkan Pengguna' : 'Merge User' }}
                                    </a>
                                @endif
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm font-medium text-gray-900">{{ $isId ? 'Belum ada pengguna yang terdaftar di jurnal ini' : 'No users enrolled in this journal' }}</p>
                                <p class="text-xs text-gray-500 mt-1">{{ $isId ? 'Daftarkan pengguna yang ada atau buat yang baru untuk memulai.' : 'Enroll existing users or create new ones to get started.' }}</p>
                                <div class="mt-4 flex gap-3">
                                    <a href="{{ route($routePrefix . '.enroll', ['journal' => $journal->slug]) }}"
                                        class="text-sm text-indigo-600 hover:text-indigo-700 font-medium">
                                        <i class="fa-solid fa-user-plus mr-1"></i> {{ $isId ? 'Daftarkan Pengguna' : 'Enroll User' }}
                                    </a>
                                </div>
                            </div>
                        </td>
