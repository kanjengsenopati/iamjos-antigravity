...
@section('title', $isId ? 'Navigasi Situs' : 'Site Navigation')
    ...
    <div class="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div>
                    <h1 class="text-xl font-bold text-gray-900">{{ $isId ? 'Navigasi Situs' : 'Site Navigation' }}</h1>
                    <p class="text-sm text-gray-500">{{ $isId ? 'Kelola menu navigasi untuk portal' : 'Manage navigation menus for the portal' }}</p>
                </div>
            ...
            @foreach($menus as $menu)
                <div x-show="activeTab === '{{ $menu->area_name }}'" x-cloak class="p-6">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <p class="text-sm text-gray-500">{{ $isId ? 'Geser item untuk menyusun ulang. Klik untuk mengedit.' : 'Drag items to reorder. Click to edit.' }}</p>
                        <button @click="openAddModal('{{ $menu->id }}')"
                                class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 cursor-pointer">
                            <i class="fa-solid fa-plus mr-2"></i>
                            {{ $isId ? 'Tambah Item Menu' : 'Add Menu Item' }}
                        </button>
                    </div>
                                ...
                                <div class="menu-item flex items-center gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-indigo-200 cursor-move"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="text-xs text-gray-500 truncate">
                                            @if($assignment->item->type === 'route')
                                                <span class="text-indigo-600">{{ $isId ? 'Rute:' : 'Route:' }}</span> {{ $assignment->item->route_name }}
                                            @elseif($assignment->item->type === 'divider')
                                                <span class="text-gray-400">— {{ $isId ? 'Pemisah' : 'Divider' }} —</span>
                                            @else
                                                {{ $assignment->item->url ?? '#' }}
                                            @endif
                                        </div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span class="text-xs px-2 py-1 rounded-full {{ $assignment->item->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                            {{ $assignment->item->is_active ? ($isId ? 'Aktif' : 'Active') : ($isId ? 'Tersembunyi' : 'Hidden') }}
                                        </span>
                                    ...
                                    <div class="flex items-center gap-1">
                                        <button @click="openEditModal('{{ $assignment->item->id }}', {{ json_encode($assignment->item) }})"
                                                class="p-2 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg cursor-pointer"
                                                title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </button>
                                        <button @click="deleteItem('{{ $assignment->item->id }}')"
                                                class="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg cursor-pointer"
                    ...
                    @else
                        <div class="text-center py-12 text-gray-500">
                            ...
                            <p>{{ $isId ? 'Belum ada item menu. Tambahkan item pertama Anda di atas.' : 'No menu items yet. Add your first item above.' }}</p>
                        </div>
                    @endif
                </div>
            @endforeach
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block w-full max-w-lg my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl relative z-10">
                ...
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 class="text-lg font-semibold text-gray-900" x-text="editingItem ? '{{ $isId ? 'Edit Item Menu' : 'Edit Menu Item' }}' : '{{ $isId ? 'Tambah Item Menu' : 'Add Menu Item' }}'"></h3>
                </div>
                ...
                <form @submit.prevent="saveItem()" class="p-6 space-y-4">
                    {{-- Label --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $isId ? 'Label' : 'Label' }} <span class="text-red-500">*</span></label>
                        <input type="text" x-model="formData.label" required
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                               placeholder="{{ $isId ? 'Label menu' : 'Menu label' }}">
                    </div>

                    {{-- Type --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $isId ? 'Tipe' : 'Type' }}</label>
                        <select x-model="formData.type"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="custom">{{ $isId ? 'Tautan Kustom' : 'Custom Link' }}</option>
                            <option value="route">{{ $isId ? 'Rute Internal' : 'Internal Route' }}</option>
                            <option value="page">{{ $isId ? 'Halaman Situs' : 'Site Page' }}</option>
                            <option value="divider">{{ $isId ? 'Pemisah' : 'Divider' }}</option>
                        </select>
                    </div>
                    ...
                    <div x-show="formData.type === 'custom'">
                        ...
                        <input type="text" x-model="formData.url"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                               placeholder="https://example.com {{ $isId ? 'atau /jalur' : 'or /path' }}">
                    </div>

                    {{-- Route (for route type) --}}
                    <div x-show="formData.type === 'route'">
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $isId ? 'Rute' : 'Route' }}</label>
                        <select x-model="formData.route_name"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="">{{ $isId ? 'Pilih rute...' : 'Select a route...' }}</option>
                    ...
                    <div x-show="formData.type === 'page'">
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $isId ? 'Halaman' : 'Page' }}</label>
                        <select x-model="formData.page_id"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="">{{ $isId ? 'Pilih halaman...' : 'Select a page...' }}</option>
                    ...
                    <div x-show="formData.type !== 'divider'">
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $isId ? 'Ikon (kelas FontAwesome)' : 'Icon (FontAwesome class)' }}</label>
                        <input type="text" x-model="formData.icon"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                               placeholder="{{ $isId ? 'misal: fa-solid fa-home' : 'e.g., fa-solid fa-home' }}">
                    </div>

                    {{-- Target --}}
                    <div x-show="formData.type !== 'divider'">
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $isId ? 'Buka di' : 'Open in' }}</label>
                        <select x-model="formData.target"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="_self">{{ $isId ? 'Jendela yang sama' : 'Same window' }}</option>
                            <option value="_blank">{{ $isId ? 'Jendela baru' : 'New window' }}</option>
                        </select>
                    </div>
                    ...
                    <div class="flex items-center gap-3" x-show="editingItem">
                        ...
                        <label for="is_active" class="text-sm font-medium text-gray-700">{{ $isId ? 'Aktif' : 'Active' }}</label>
                    </div>
                </form>
                ...
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-end gap-3">
                    ...
                    <button @click="saveItem()" type="button"
                            ...
                            :disabled="saving">
                        <span x-show="!saving" x-text="editingItem ? '{{ $isId ? 'Perbarui' : 'Update' }}' : '{{ $isId ? 'Tambah Item' : 'Add Item' }}'"></span>
                        <span x-show="saving"><i class="fa-solid fa-spinner fa-spin mr-2"></i>{{ $isId ? 'Menyimpan...' : 'Saving...' }}</span>
                    </button>
                </div>
            </div>
        </div>
...
function navigationManager() {
    return {
        ...
        async deleteItem(itemId) {
            if (!confirm('{{ $isId ? 'Apakah Anda yakin ingin menghapus item menu ini?' : 'Are you sure you want to delete this menu item?' }}')) return;
